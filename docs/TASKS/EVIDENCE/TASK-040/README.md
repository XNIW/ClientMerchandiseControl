...
## Fix 37 — evidence corrente

- exact technical SHA `102caa49070c7870234119f31d142c79965b1fd2`;
- parent privato mantenuto descriptor-bound nel guard; rename/delete/revoke
  dell'ancestor e `KQ_NOTE_ATTRIB` sui descriptor dell'albero sono osservati;
- confronto finale device/inode/mode/ctime chiude parent e chmod ABA senza
  invalidare le letture nominali del validator;
- canale guard aperto prima del child e cleanup tramite quarantine random
  identity-bound chiudono early-exit, collisione e victim-swap;
- candidate reale 686/207 `PASS`, unsigned; tree SHA-256
  `88808438585983a997fbab3c5c54850e559dd7698ac3f0df73604cb8d82aaf2d`
  e sealed payload SHA-256
  `d1e63a6f7ed75d52b9155563e503ed2c43d2066f00d8dac9e987b928ce13e10f`;
- validator iOS 73/73, Flutter/YAML 12/12, governance 88/88, security
  61/61 negative + 7/7 positive, architecture 17/17, analyze e format `PASS`;
- `scripts/check.sh` exit 0: 804/804 non-performance, 10/10 performance,
  repeat 5 x 14 = 70/70, Android debug e iOS Simulator debug `PASS`;
...
## Fix 38 — evidence corrente

- exact technical SHA `96fe31f8b149220a87b6bcb0a64c2f63875fd4e5`;
- l'intera ancestor chain della root privata resta aperta, sorvegliata e
  riattestata; l'ABA sopra il parent immediato viene respinto;
- cleanup temp-root component-bound e identity-bound prima dei marker; il
  confronto `lstat`/`fstat` lega anche ogni child directory aperto;
- regressioni ancestor ABA, root victim swap e child `stat→open` `PASS`;
- candidate reale 686/207 `PASS`, unsigned; tree SHA-256
  `88808438585983a997fbab3c5c54850e559dd7698ac3f0df73604cb8d82aaf2d`
  e sealed payload SHA-256
  `d1e63a6f7ed75d52b9155563e503ed2c43d2066f00d8dac9e987b928ce13e10f`;
- validator iOS 76/76, Flutter/YAML 12/12, governance 88/88, security
  61/61 negative + 7/7 positive, architecture 17/17, analyze e format `PASS`;
- `scripts/check.sh` exit 0: 804/804 non-performance, 10/10 performance,
  repeat 5 x 14 = 70/70, Android debug e iOS Simulator debug `PASS`;
- upload-ready validator exit 1 con
...
- verdict prodotto `CHANGES_REQUIRED`: P0 0 / P1 0 / P2 0 / P3 3;
  security deduplicato P3 1;
- child post-clear `close→rmdir`, root pre-identity e stat/rename quarantine
  conservano schedule name-based che possono cancellare o spostare victim;
- il cleanup nominale lascia un tombstone per invocazione, con 100 totali e
  51 recenti osservati dal reviewer;
- closure reviewer: ancestor-chain ABA, temp-root swap post-identity e child
  `stat→open` `PASS`;
- gate reviewer: iOS 76/76, candidate 686/207, Flutter/YAML 12/12,
  governance 88/88, architecture, scanner, analyze e syntax `PASS`;
- report security sealed SHA-256
  `7f25fcfbf8c5702c8424a5c040b3824f600090a69c332b99d17abbe7da27c515`;
  production, signing, provisioning e TestFlight invariati.

## Fix 39 — evidence corrente

- exact technical SHA `8b820b96f6f3f288c2194bf0ca2a1dbbd2c5bdbe`;
- creazione root temporanea e cattura `device,inode` avvengono nello stesso
  helper sotto lock esclusivo; path, parent e prefisso vengono verificati dal
  validator prima dell'uso;
- cleanup identity-bound senza `unlink`/`rmdir`: quarantine esclusiva
  `RENAME_EXCL`, confronto inode/mode, truncate tramite descriptor e rifiuto di
  hardlink o collisioni;
- retention esplicitamente bounded a 512 entry per parent e fail-closed; le
  nuove root Fix 39 ispezionate contengono zero file con payload, senza
  cancellare i tombstone storici appartenenti ai fix precedenti;
- regressioni root pre-identity, post-clear swap, no-name-based-terminal,
  hardlink e cap `PASS`; validator iOS 80/80 e Flutter/YAML 12/12;
- candidate reale source/artifact 686/207 `PASS`, unsigned; tree SHA-256
  `88808438585983a997fbab3c5c54850e559dd7698ac3f0df73604cb8d82aaf2d`
  e sealed payload SHA-256
  `d1e63a6f7ed75d52b9155563e503ed2c43d2066f00d8dac9e987b928ce13e10f`;
- `scripts/check.sh` exit 0: 804/804 non-performance, 10/10 performance,
  repeat 70/70, security source 686, fixture 61/61 + 7/7, governance 88/88,
  architecture 17/17, analyze/format e build debug Android/iOS `PASS`;
...
## Fix 40 — evidence corrente

- exact technical SHA `7fdd29a6ffd8f10d2c6a065cc30ad5c4220f4ff1`;
- il cleanup ricontrolla `st_nlink == 1` sul descriptor dopo la quarantine,
  immediatamente prima del truncate e dopo la bonifica; il late-hardlink
  viene respinto e il payload resta preservato;
- la creazione delle root usa il lock esclusivo del descriptor della directory
  parent, con verifica dell'intera ancestor chain prima e dopo la sezione
  critica; la regressione interprocess legacy-lock ABA con cap 1 consente una
  sola root;
- le entry retained sono legate a `device,inode,mode` e, per i file, a
  `nlink == 1` e size zero; replacement post-quarantine file e directory
  falliscono chiusi;
- parent con tab/newline viene respinto prima della creazione della root;
  nel `TMPDIR` locale condiviso erano presenti 416 tombstone su cap 512:
  le esecuzioni nominali lasciano solo tombstone zero-payload, mentre le
  fixture avversariali che falliscono preservano intenzionalmente i victim;
  nessun residuo storico è stato cancellato e la re-review lunga deve usare
  un `TMPDIR` privato nuovo;
...
- `scripts/check.sh` exact technical SHA exit 0: 804/804 non-performance,
  performance 10/10, repeat 70/70, Android debug e iOS Simulator debug
  `PASS`;
...
## Fix 44 — blocker corrente

- exact technical SHA `3d9dd674d0fff7566345afbb42de99f486fa1ab0`;
- il parser globale rimuove fino a tre spazi CommonMark prima di leggere la
  riga e rifiuta duplicate TASK-040 indentate 1, 2 e 3 spazi;
- cardinalità globale/canonica e stato header restano legati; le tre mutazioni
  di stato Fix 43 continuano a fallire per la ragione esatta;
- `bash -n`, checker canonico, governance 94/94, action pins e diff-check
...
## Fix 45 — blocker corrente

- exact technical SHA `825a45a221bb67e9c4678cde90c123b57efe0d62`;
- policy unica per role task/manifest e forma worklog in base a fase+indicatore;
- `CODEX_REVIEW_BLOCKED` usa Re-review/review/Exact HEAD, mentre
  `CODEX_FIX_BLOCKED_TO_RE_REVIEW` mantiene Fix/technical/Technical SHA;
- regressioni policy nelle tre combinazioni previste; governance 97/97,
  syntax, pins e diff-check `PASS`;
...
- retained file/directory/FIFO descriptor-bound, verifica ricorsiva zero-payload
  e due pass terminali; replacement file, directory e child same-inode vengono
  rilevati preservando il payload avversariale;
- cap verificato due volte dopo `mkdir`; l'iniezione count-to-create con cap 1
  fallisce, non restituisce record e conserva soltanto la entry avversariale;
- lock oracle esatto `LOCK_EX`; nessun `unlink`/`rmdir` cleanup name-based;
- suite finale iOS 86/86, Flutter/YAML 12/12, governance 88/88, security
  61/61 negative + 7/7 positive, architecture 17/17, analyze e format `PASS`;
- candidate reale source/artifact 686/207 `PASS`, unsigned; tree SHA-256
  `88808438585983a997fbab3c5c54850e559dd7698ac3f0df73604cb8d82aaf2d`
  e sealed payload SHA-256
