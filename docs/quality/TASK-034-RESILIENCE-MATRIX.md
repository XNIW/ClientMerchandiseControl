...
- `REPEAT-120`: 10 iterazioni per 12 race client critiche, 120 esecuzioni, exit `0`.
...
| Auth | logout con request in corso / cancellazione | critical | account generation e compensazione sign-out | completion vecchia ignorata, sessione purgata | `CLIENT-314`, `REPEAT-120`: auth controller | PASS |
...
| Auth | callback/deep link prima, durante, dopo auth o duplicato | high | callback PKCE canonico e flow token | cold fuori flow ignorato; duplicato consumato una volta; expiry 5 min | `CLIENT-314`, `REPEAT-120`; scheduler manuale | PASS |
...
| Catalogo | query o category/filter precedente completa dopo nuova | high | generation per query/filter | completion precedente ignorata | `CLIENT-314`, `REPEAT-120`: catalog controller | PASS |
...
| Carrello | add/remove/update concorrenti e doppio tap | critical | RPC server; idempotency key + expected version | coda seriale, un solo side effect, conflict esplicito | `CLIENT-314`, `DB-483`, `DB-RACE-11`, `REPEAT-120` | PASS |
...
| Carrello | duplicate key o stale response | critical | idempotency + expected version/generation | replay senza side effect; risposta account vecchio scartata | `CLIENT-314`, `DB-RACE-11`, `REPEAT-120` | PASS |
| Checkout | doppio submit | critical | create-order RPC/idempotency | richiesta in-flight condivisa; un solo ordine | `CLIENT-314`, `DB-483`, `DB-RACE-11`, `REPEAT-120` | PASS |
...
| Ordini | cancellation duplicate/timeout ambiguo | critical | cancel RPC/key/expected status version | stessa key al retry; release/outbox una volta | `CLIENT-314`, `DB-483`, `DB-RACE-11`, `REPEAT-120` | PASS |
...
| Tracking | terminal event race e redaction | critical | lifecycle RPC/state machine | terminale prevale e la posizione non riappare | `CLIENT-314`, `DB-483`, `REPEAT-120` | PASS |
...
| Tracking | logout/account switch/background/foreground | critical | auth generation + route lifecycle | teardown/cache purge; resume rearma una sola runtime | `CLIENT-314`, Fix tracking `22/22`, `REPEAT-120`: A→null/A→B/dispose con fallback e unsubscribe asincroni; zero task residui o provider read post-dispose | PASS |
...
Le dodici race client critiche sono state ripetute 10 volte (`REPEAT-120`) con `Completer`,
